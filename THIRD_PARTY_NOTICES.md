...
本地演示代码为独立编写；**没有复制、打包、启动或调用下列仓库的源码**，也没有复现官方生产请求格式。
本项目不是两个上游仓库的合并版，不能声称已经完成真实系统集成。
...
- 本次处理：仅注明来源，不复制其实现、图片、提取的小程序代码或其他素材。公开可读不自动等于可以任意复制或重新分发；后续如需引入代码，应先核实作者授权和素材权利。
